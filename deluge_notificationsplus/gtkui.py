...
class GtkUiNotifications(CustomNotifications):
    ...
    def enable(self):
        ...
        self.register_custom_blink_notification(
            'TorrentAddedEvent', self._on_torrent_finished_event_blink
        )
        self.register_custom_sound_notification(
            'TorrentAddedEvent', self._on_torrent_finished_event_sound
        )
        self.register_custom_popup_notification(
            'TorrentAddedEvent', self._on_torrent_added_event_popup
        )
    ...
    def disable(self):
        ...
        self.deregister_custom_blink_notification('TorrentAddedEvent')
        self.deregister_custom_sound_notification('TorrentAddedEvent')
        self.deregister_custom_popup_notification('TorrentAddedEvent')
    ...
    def _on_torrent_finished_event_blink(self, torrent_id, from_state=None):
        return True  # Yes, Blink

    def _on_torrent_finished_event_sound(self, torrent_id, from_state=None):
    ...
    def _on_torrent_added_event_popup(self, torrent_id, from_state):
        d = client.core.get_torrent_status(torrent_id, ['name', 'file_progress'])
        d.addCallback(self._on_torrent_added_event_got_torrent_status)
        d.addErrback(self._on_torrent_finished_event_torrent_status_failure)
        return d
    ...
    def _on_torrent_added_event_got_torrent_status(self, torrent_status):
        log.debug(
            'Handler for TorrentAddedEvent GTKUI called. ' 'Got Torrent Status'
        )
        title = _('Added Torrent')
        torrent_status['num_files'] = torrent_status['file_progress'].count(1.0)
        message = (
            _(
                'The torrent "%(name)s" including %(num_files)i file(s) '
...
class GtkUI(Gtk3PluginBase, GtkUiNotifications):
        ...
        self.builder.connect_signals(
            {
                ...
                'on_tg_enabled_toggled': self.on_tg_enabled_toggled,
    ...
    def build_notifications_model_populate_treeview(self):
        ...
        self.subscriptions_model = Gtk.ListStore(str, str, bool, bool, bool, bool, bool)
        ...
        renderer = Gtk.CellRendererToggle()
        renderer.set_property('activatable', True)
        renderer.connect('toggled', self._on_tg_col_toggled)
        column = Gtk.TreeViewColumn('Telegram', renderer, active=SUB_NOT_TG)
        column.set_clickable(True)
        self.subscriptions_treeview.append_column(column)
    ...
    def popuplate_what_needs_handled_events(
        self, handled_events, email_subscriptions=None, tg_subscriptions=None
    ):
        if email_subscriptions is None:
            email_subscriptions = []
        if tg_subscriptions is None:
            tg_subscriptions = []
        self.populate_subscriptions(handled_events, email_subscriptions, tg_subscriptions)
    ...
    def populate_subscriptions(self, handled_events, email_subscriptions=None, tg_subscriptions=None):
        if email_subscriptions is None:
            email_subscriptions = []
        if tg_subscriptions is None:
            tg_subscriptions = []
        ...
        for event_name, event_doc in handled_events:
            self.subscriptions_model.set(
                ...
                SUB_NOT_TG,
                event_name in tg_subscriptions
            )
    ...
    def on_apply_prefs(self):
        ...
        current_telegram_subscriptions = []
        for event, doc, email, popup, blink, sound, telegram in self.subscriptions_model:
            if email:
            ...
            if telegram:
                current_telegram_subscriptions.append(event)
        ...
        core_config = {
            ...
            'subscriptions': {'email': current_email_subscriptions, 'telegram': current_telegram_subscriptions},
            'tg_enabled': self.builder.get_object('tg_enabled').get_active(),
            'tg_bot_token': self.builder.get_object('tg_bot_token').get_text(),
    ...
    def cb_get_config(self, core_config):
        ...
        self.builder.get_object('tg_bot_token').set_text(core_config['tg_bot_token'])
        self.builder.get_object('tg_chat_id').set_text(core_config['tg_chat_id'])
        ...
        self.builder.get_object('tg_enabled').set_active(core_config['tg_enabled'])
    ...
    def on_tg_enabled_toggled(self, widget):
        for widget_name in (
            'tg_bot_token',
            'tg_chat_id',
        ):
    ...
    def _on_tg_col_toggled(self, cell, path):
        self.subscriptions_model[path][SUB_NOT_TG] = not self.subscriptions_model[
            path
        ][SUB_NOT_TG]
        return
